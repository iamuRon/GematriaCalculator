...
class MinerWorker(QObject):
    ...
    def cpu_process_boundary_chunk(self, middle_combos, length):
        ...
        for i, middle in enumerate(middle_combos):
            ...
            if not self.is_running:
                break
            word = self.first_char + middle + self.last_char
            if len(word) != length:
            ...
            if is_prime(val):
                ...
                last_word = word
            if i % 1000 == 0 and i > 0:
                self.result_signal.emit([], length, self.first_char + middle + self.last_char, self.prime_position)
    ...
    def gpu_process_boundary_chunk(self, indices, length):
        ...
        try:
            ...
            return self.cpu_process_boundary_chunk(
                [''.join(self.charset[i % self.charset_size] for _ in range(length - 2)) for i in indices], length
            )
        except Exception as e:
                ...
                for start in range(0, combos_count, chunk_size):
                    ...
                    for i in range(start, end):
                        ...
                        for _ in range(middle_length):
                            char_idx = idx % self.charset_size
                            middle.append(self.charset[char_idx])
                            idx //= self.charset_size
                        middle_combos.append(''.join(middle))
                    if use_gpu:
...
class Base10Checker(QObject):
    ...
    def __init__(self, base22_results, temp_file):
        ...
        self.temp_file = temp_file
        self.is_running = True
        self.base10_buffer = deque()  # Store Base-10 results
        self.entry_count = 0
    ...
    def update_temp_file(self, new_results):
        try:
            ...
            for result in new_results:
                ...
                self.entry_count += 1
            print(f"Appended {len(new_results)} results to base10_buffer. Current buffer size: {len(self.base10_buffer)}")
        except Exception as e:
            self.error_signal.emit(f"Failed to write to temp file: {str(e)}")
        ...
        try:
            ...
            for i, entry in enumerate(self.base22_results):
                ...
                try:
                    ...
                    continue
            # Ensure any remaining results are processed
...
class MinerTab(QWidget):
    ...
    def extract_boundary_params(self, boundary_text):
        """Extract first char, last char, and total length from boundary input."""
        if boundary_text == "Use - and _ to set mining params":
            return None, None, 0

        # Filter Hebrew characters to determine first and last
        ...
        total_length = len(boundary_text)  # Count all chars including _ and -
    ...
    def start_mining(self):
        ...
        self.cleanup_thread()  # Clear previous results before starting new batch
